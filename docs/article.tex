...
Tabela~\ref{tab:mbert_fewshot} prikazuje rezultate fine-tuning-a mBERT modela na različnih velikostih few-shot naborov.
...
Tabela~\ref{tab:tinybert_fewshot} prikazuje rezultate fine-tuning-a TinyBERT modela na različnih velikostih few-shot naborov. TinyBERT je znatno manjši model, namenjen mobilnim aplikacijam.
...
Za boljšo razlago razlik med natančnostjo in priklicem prikazujemo matrike zmede. V nadaljevanju interpretiramo predvsem \textit{FP} (ham $\rightarrow$ spam) in \textit{FN} (spam $\rightarrow$ ham), saj neposredno pojasnjujeta spremembe v natančnosti in priklicu.
