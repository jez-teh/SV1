...
\begin{abstract}
    ...
    modelov. Raziskava replicira in nadgradi metodologijo iz sodobnih študij z vključitvijo slovenščine 
    ...
    mobilne naprave. Modele naučimo na angleških podatkih in jih vrednotimo na slovenskih podatkih, 
...
\section{Uvod}
    ...
    Problem detekcije neželenih SMS sporočil se pomembno razlikuje od klasifikacije elektronske pošte in 
    drugih oblik spletne komunikacije zaradi specifičnih lastnosti samega medija. SMS sporočila so bistveno 
    krajša, pogosto vsebujejo neformalni jezik, kratice, fonetične zapise, emojije ter namerne pravopisne napake, 
    ki jih pošiljatelji uporabljajo za obhod filtrov~\cite{lota2017systematic}. Zaradi omejene dolžine 
    besedila imajo modeli na voljo manj semantičnega konteksta kot pri elektronski pošti, kjer so prisotni daljši stavki, 
    zadeve sporočil, HTML struktura in metapodatki~\cite{Jain2018SpamDO}. Poleg tega SMS komunikacija pogosto vključuje 
    mobilno-specifične značilnosti, kot so telefonske številke, časovni vzorci pošiljanja in omrežni podatki, ki 
    jih pri elektronski pošti ni ali pa imajo drugačno strukturo~\cite{6133257}. Raziskave kažejo tudi, da se spam v 
    SMS okolju hitreje prilagaja filtrom z uporabo kratkih dinamičnih sporočil in socialnega inženiringa, zaradi 
    česar je generalizacija modelov težja kot pri tradicionalni e-pošti~\cite{Jain2018SpamDO}. Zaradi teh 
    razlogov metode, razvite za detekcijo neželene elektronske pošte, pogosto niso neposredno prenosljive na SMS 
    domeno brez dodatnih prilagoditev modelov in predobdelave besedila.
...
\begin{enumerate}
    ...
    \item \textbf{Vizualizacija in analiza}: Za vse kombinacije modelov in pristopov smo na testni množici izračunali metrike (točnost, natančnost, priklic in F1), pri čemer smo pozitivni razred definirali kot \emph{spam}. Poleg agregiranih metrik smo generirali tudi konfuzijske matrike, da smo lahko ločeno analizirali \textit{FP} (ham $\rightarrow$ spam) in \textit{FN} (spam $\rightarrow$ ham) ter tako razložili spremembe v natančnosti/priklicu. Za lažjo primerjavo med modeli smo pripravili še grafe metrik v odvisnosti od $k$, kar omogoča vizualno interpretacijo ne-monotonega obnašanja pri few-shot prilagajanju.
\end{enumerate}
...
\caption{Primerjava točnosti (accuracy) med mBERT in TinyBERT glede na število few-shot primerov. Ker je razred \textit{ham} pogostejši, ostane točnost visoka tudi pri scenarijih, kjer se priklic spama poslabša; zato je pri interpretaciji pomembno spremljati tudi priklic in F1.}
...
\caption{Primerjava natančnosti (precision) med mBERT in TinyBERT glede na število few-shot primerov. Pri TinyBERT se natančnost pri nekaterih $k$ poveča (manj lažno pozitivnih), vendar to ne pomeni nujno boljšega F1, če se hkrati zmanjša priklic.}
...
\caption{Primerjava priklica (recall) med mBERT in TinyBERT glede na število few-shot primerov. Padec F1 pri TinyBERT pri $k=10$ in $k=200$ sovpada z opaznim padcem priklica (več zgrešenih \textit{spam} primerov), kar potrjuje, da je glavni problem pri teh $k$ povečanje lažno negativnih napovedi.}
...
\subsubsection{Konfuzijske matrike}

Za boljšo razlago razlik med natančnostjo in priklicem prikazujemo konfuzijske matrike. V nadaljevanju interpretiramo predvsem \textit{FP} (ham $\rightarrow$ spam) in \textit{FN} (spam $\rightarrow$ ham), saj neposredno pojasnjujeta spremembe v natančnosti in priklicu.
...
\caption{Konfuzijska matrika TinyBERT pri few-shot $k=200$. Opazimo poslabšanje glede na $k=100$: $FN=452$ ponovno naraste (več spregledanega spama), $FP=95$ pa ostane relativno nizek. To kaže, da več slovenskih primerov v few-shot režimu ne prinese nujno stabilnejšega modela, temveč lahko potisne odločanje v smer, kjer model sicer ne dela veliko lažnih alarmov, a hkrati prepogosto spregleda spam, kar zniža F1.}
...
\begin{itemize}
    ...
    \item \textbf{Domenske razlike:} Izkušeni SMS sporočila v angleščini in slovenščini lahko vsebujejo različne vzorce spama, kar bi lahko vplivalo na rezultate.
...
Rezultati potrjujejo, da mBERT izkazuje zelo stabilen medjezikovni prenos znanja: F1 na slovenskem testnem naboru v zero-shot scenariju ostane primerljiv z rezultatom v scenariju EN $\rightarrow$ EN, few-shot prilagajanje pa prinese le minimalne spremembe. To kaže, da lahko za visoko natančno detekcijo spama v slovenščini v praksi pogosto zadostuje že model, naučen na angleških podatkih.
